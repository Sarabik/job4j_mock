...
public class IndexController {
    ...
    public String getIndexPage(Model model, HttpServletRequest req) throws JsonProcessingException {
        ...
        Map<InterviewDTO, ProfileDTO> interviewsMap = interviewsService.getByType(1)
                .stream()
                .collect(Collectors.toMap(
                        interviewDTO -> interviewDTO,
                        interviewDTO -> profilesService.getProfileById(interviewDTO.getSubmitterId()).get()
                        ));
        model.addAttribute("interviewsMap", interviewsMap);
        return "index";
    }
}